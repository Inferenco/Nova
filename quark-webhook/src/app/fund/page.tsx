...
export default function FundPage() {
  const { account, connected, signTransaction, submitTransaction } =
    useWallet();
  ...
  const handleFund = async () => {
    ...
    try {
      ...
      const rawAmount = Math.floor(fundAmount * Math.pow(10, decimals));

      // Get the resource account address first
      ...
      if (!resourceAccountResponse || !resourceAccountResponse[0]) {
      ...
      const transaction = await aptos.transferCoinTransaction({
        ...
        recipient: resourceAccountAddress,
        amount: rawAmount,
        coinType: selectedCoin.asset_type as `${string}::${string}::${string}`,
      });

      const txPayload = await signTransaction({
        transactionOrPayload: transaction,
      });

      const response = await submitTransaction({
        senderAuthenticator: txPayload.authenticator,
        transaction,
      });

      if (response) {
        showMessage(
          `Successfully funded ${amount} ${getCoinSymbol(selectedCoin)}`,
          "success"
        );
        setAmount("");
        setSelectedCoin(null);
        // Refresh coins after successful funding
        await fetchCoins();
      } else {
      ...
      showMessage("Funding failed: " + (error as Error).message, "error");
    } finally {
      setFunding(false);
  ...
  return (
    <Page>
      ...
      <div style={{ padding: "10px 10px 20px 10px" }}>
        ...
        >
          Select a token and amount to fund your Quark resource account
        </Caption>
      </div>
                ...
                <div style={{ padding: "20px" }}>
                  ...
                  >
                    {funding
                      ...
                      : isDelaying
                      ? "Please wait..."
                      : `Fund ${getCoinSymbol(selectedCoin)}`}
                  </Button>
                </div>
              </Card>
            </Section>
